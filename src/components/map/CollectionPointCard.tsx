...
const CollectionPointCard = ({
  ...
  return <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 border border-gray-200 dark:border-gray-700 mb-4">
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        <a href={`https://www.google.com/maps/search/?api=1&query=${point.latitude},${point.longitude}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-xs bg-white-100 text-white-800 rounded-full hover:bg-white transition-colors mx-0 py-[5px] px-[2px]">
          <img src={GoogleMapsLogo} alt="Google Maps" className="w-8 h-6" />
        </a>
        <a href={`https://waze.com/ul?ll=${point.latitude},${point.longitude}&navigate=yes`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-xs bg-white text-gray-800 rounded-full hover:bg-white transition-colors mx-0 py-[5px] px-[6px]">
